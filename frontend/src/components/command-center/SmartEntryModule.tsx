...
export default function SmartEntryModule() {
  ...
  const setGeneratedContent = useResearchStore(
    (state) => state.setGeneratedContent,
  );
  ...
  const detectedImageURL = useMemo(
    () =>
      inputMode === "link" && inputValue.trim() && isImageURL(inputValue.trim())
        ? inputValue.trim()
        : null,
  ...
  const [aiSuggestions, setAiSuggestions] =
    useState<AutoSuggestResponse | null>(null);
  ...
  useEffect(() => {
    if (userPrefs && !prefsApplied) {
      setBriefForm((prev) => ({
        ...
        target_audience:
          userPrefs.default_target_audience || prev.target_audience,
    ...
    if (activeJob?.pages) {
      activeJob.pages.forEach((page, i) => {
        items.push({
          ...
          content:
            page.markdown_text || page.extracted_text || "No content extracted",
          url: page.url,
  ...
  useEffect(() => {
    if (previewOpen && previewItems.length > 0) {
      if (
        !selectedPreviewId ||
        !previewItems.find((i) => i.id === selectedPreviewId)
      ) {
  ...
  const activePreviewItem =
    previewItems.find((i) => i.id === selectedPreviewId) || previewItems[0];
  ...
  const handleAnalyze = useCallback(
    async (forcedStrategy?: "auto" | "image", isCombined?: boolean) => {
      ...
      try {
        ...
        const payload: StartResearchRequest = {
          ...
          image_url:
            isCombined || isVisionOnly ? imageUrlValue.trim() : undefined,
          strategy: strategy,
        ...
        if (isCombined) {
          gooeyToast.success(
            "Combined Research started — Crawling web + analyzing image",
          );
        } else {
          gooeyToast.success(
            isVision
              ? "Image analysis job created — analyzing with LLaVA"
              : "Crawl job created",
          );
        }
      } catch (error: any) {
  ...
  const handleGenerateOutline = async () => {
    ...
    if (!knowledgeText) {
      gooeyToast.error(
        "No knowledge source data available. Please crawl a URL first.",
      );
      return;
    }
    ...
    try {
      ...
      setGenerationStage(
        "Đang áp dụng Framework " +
          (briefForm.framework || "AIDA").toUpperCase() +
          "...",
      );
      ...
      const payload: GenerateOutlineRequest = {
        ...
        additional_instructions: briefForm.additional_instructions
          ? `${briefForm.additional_instructions}\n\nNote: This outline will be used to generate content for Facebook, LinkedIn, and Blog simultaneously.`
          : "This outline will be used to generate content for Facebook, LinkedIn, and Blog simultaneously.",
        language: briefForm.language,
      };
      ...
      const result = await researchApi.generateOutline(
        payload,
        controller.signal,
      );
      ...
      try {
        ...
        const sectionsArray =
          parsed.sections || (Array.isArray(parsed) ? parsed : []);
        if (Array.isArray(sectionsArray)) {
          ...
          const transformed = sectionsArray.map((item: any, idx: number) => ({
            ...
            level: 1,
            text:
              item.heading || item.section || item.title || "Untitled Section",
            children: (item.key_points || item.points || []).map(
      ...
      gooeyToast.success(
        "Outline generated! Review and edit before generating content.",
      );
    } catch (error: any) {
  ...
  const getPlatformContentLength = (
    platform: string,
    userChoice: string,
  ): string => {
    ...
    switch (platform) {
      case "facebook":
        return "short";
      case "linkedin":
        return userChoice === "long" ? "medium" : userChoice;
      case "blog":
        return userChoice;
      default:
        return userChoice;
  ...
  const getPlatformInstructions = (
    ...
    const platformHints: Record<string, string> = {
      facebook:
        "Optimize for Facebook: Use hook in first 2 lines, add emojis strategically, keep paragraphs very short (2-3 sentences), end with hashtags (3-5). Format for mobile-first reading.",
      linkedin:
        "Optimize for LinkedIn: Lead with a bold insight or stat, use professional but conversational tone, add line breaks between paragraphs, include data points, end with an engaging question.",
  ...
  const resolveImageMetadata = useCallback((): {
    emotion: string;
    context: string;
  } => {
  ...
  const handleGenerateFromOutline = async () => {
    ...
    try {
      ...
      for (let i = 0; i < ALL_PLATFORMS.length; i++) {
        ...
        setGenerationStage(
          `Đang tạo nội dung ${platform === "blog" ? "Blog" : platform === "facebook" ? "Facebook" : "LinkedIn"}...`,
        );
        ...
        try {
          const payload: GenerateContentRequest = {
            ...
            target_audience:
              briefForm.target_audience ||
              "Người quan tâm đến công nghệ và marketing",
            content_length: getPlatformContentLength(
              platform,
              briefForm.content_length,
            ),
            additional_instructions: getPlatformInstructions(
              platform,
              stylePrompt,
            ),
            language: briefForm.language,
          ...
          gooeyToast.error(
            `${platform} generation failed: ${error?.message || "Unknown error"}`,
          );
        }
      }
      ...
      if (successCount > 0) {
        gooeyToast.success(
          `Generated content for ${successCount}/${totalPlatforms} platforms! Check the Editor panel.`,
        );
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            <Button
              variant="ghost"
              size="sm"
              className="h-7 px-2 text-[10px] text-label hover:text-primary"
            ...
            {inputMode === "file" ? (
              ...
              <div className="flex flex-col">
                  ...
                  <>
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && handleAnalyze("image")
                        }
                      ...
                      <Button
                        size="sm"
                        className="h-6 px-2 mr-1.5 text-[10px] border-0 bg-primary/20 hover:bg-primary/30 text-primary"
                ...
                {(detectedImageURL ||
                  (inputMode === "link" &&
                    imageUrlValue.trim() &&
                    isImageURL(imageUrlValue.trim()))) && (
                  <div className="mx-2 mb-2 p-2 rounded-lg border border-primary/20 bg-primary/[0.05] flex items-center gap-2.5">
                    <div className="w-12 h-12 rounded-md overflow-hidden border border-default/50 bg-surface-hover shrink-0">
                      <img
                        src={
                          imageUrlValue.trim() || (detectedImageURL as string)
                        }
                        alt="Preview"
                ...
                {inputMode === "link" &&
                  ...
                  imageUrlValue.trim() && (
                    <div className="px-1.5 pb-1.5">
                      <Button
                        className="w-full h-8 text-[10px] bg-primary hover:bg-primary/90 text-white border-0 shadow-lg shadow-primary/20"
              ...
              className={`rounded-lg border p-3 space-y-2 min-w-0 ${
                currentJob.strategy === "image"
                  ...
                  : currentJob.source_image_url &&
                      (currentJob.strategy === "auto" ||
                        currentJob.strategy === "browser")
                    ? "border-primary/20 bg-primary/[0.03]"
                    : "border-default bg-surface-hover"
              }`}
            >
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[11px] text-body line-clamp-1 font-medium">
                        {currentJob.strategy === "image"
                          ? currentJob.title || "Analyzing image..."
                          : "Web + Image Synthesis"}
                      </p>
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-dim">
                  {currentJob.strategy === "image"
                    ? "Vision AI"
                    : `${currentJob.pages_crawled} page(s) ${currentJob.source_image_url ? "+ Vision" : ""}`}
                </span>
                <Button
                  variant="ghost"
                  size="sm"
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    >
                      <path
                        style={{ fill: "#CFF09E" }}
                        d="M256,137.646L256,137.646c-17.513,0-31.71-14.197-31.71-31.71V46.237 c0-17.513,14.197-31.71,31.71-31.71l0,0c17.513,0,31.71,14.197,31.71,31.71v59.699C287.71,123.449,273.513,137.646,256,137.646z"
                      ></path>
                      <g>
                        <path
                          style={{ fill: "#507C5C" }}
                    ...
                    >
                      ...
                      >
                        <rect
                          x="125.735"
                          y="89.163"
                          className="fill-emerald-500 dark:fill-[#CFF09E]"
                          width="150.948"
                          height="57.367"
                        ></rect>
                        <path
                          className="fill-emerald-700 dark:fill-[#507C5C]"
                          d="M276.677,160.868H125.729c-7.921,0-14.342-6.421-14.342-14.342V89.159 c0-7.921,6.421-14.342,14.342-14.342h150.948c7.921,0,14.342,6.421,14.342,14.342v57.367 C291.018,154.446,284.596,160.868,276.677,160.868z M140.071,132.184h122.264v-28.684H140.071V132.184z"
                        ></path>
                        <rect
                          x="125.735"
                          y="227.318"
                        ...
                        <g>
                          <path
                            className="fill-emerald-700 dark:fill-[#507C5C]"
                            d="M386.272,299.025H125.729c-7.921,0-14.342-6.421-14.342-14.342v-57.367 c0-7.921,6.421-14.342,14.342-14.342h260.544c7.921,0,14.342,6.421,14.342,14.342v57.367 C400.614,292.605,394.192,299.025,386.272,299.025z M140.071,270.342h231.86v-28.684h-231.86V270.342z"
                          ></path>
                          <path
                            className="fill-emerald-700 dark:fill-[#507C5C]"
                            d="M386.272,437.185H125.729c-7.921,0-14.342-6.421-14.342-14.342v-57.367 c0-7.921,6.421-14.342,14.342-14.342h260.544c7.921,0,14.342,6.421,14.342,14.342c0,7.921-6.421,14.342-14.342,14.342H140.071 v28.684h246.202c7.921,0,14.342,6.421,14.342,14.342C400.614,430.764,394.192,437.185,386.272,437.185z"
                          ></path>
                        </g>
                        <polygon
                          className="fill-emerald-500 dark:fill-[#CFF09E]"
                          points="337.748,14.342 333.923,14.342 333.923,120.949 452.067,120.949 "
                        ></polygon>
                        <path
                          className="fill-emerald-700 dark:fill-[#507C5C]"
              ...
              {previewItems.map((item) => (
                ...
                >
                  <div className="flex items-center gap-1.5 min-w-0">
                    <item.icon
                      className={`w-3.5 h-3.5 shrink-0 ${selectedPreviewId === item.id ? "text-primary" : "text-dim"}`}
                    />
                    <span
                      className={`text-xs font-medium truncate ${selectedPreviewId === item.id ? "text-heading" : "text-body"}`}
                    >
                      {item.title}
                    </span>
                  </div>
                  <span className="text-[10px] text-faint ml-5">
                    {item.type}
                  </span>
                </button>
              ))}
              ...
              {activePreviewItem ? (
                  ...
                  <div className="shrink-0 border-b border-default bg-surface-1 px-4 md:px-5 py-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-2 min-w-0">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      {previewMode !== "preview" && (
                        <button
                          onClick={() => {
                            if (!isGenerating) setPreviewMode("preview");
                          }}
                      ...
                      <h2 className="text-sm font-semibold text-heading truncate">
                        ...
                        {previewMode === "configure" &&
                          "AI Content Pipeline — Configure Brief"}
                        {previewMode === "outline" &&
                          "AI Content Pipeline — Master Outline"}
                      </h2>
                      {previewMode === "preview" && (
                        <Badge
                          variant="secondary"
                          className="text-[10px] bg-surface-active h-5 border-default whitespace-nowrap text-dim shrink-0"
                        >
                          ~{Math.round(activePreviewItem.content.length / 4)}{" "}
                          tokens
                        </Badge>
                      )}
                      {previewMode !== "preview" && (
                        <div className="flex items-center gap-1.5 shrink-0">
                          <span
                            className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold ${previewMode === "configure" ? "bg-primary text-white ring-2 ring-primary/30" : "bg-[#6FCF97] text-white"}`}
                          >
                            1
                          </span>
                          <span className="w-3 h-px bg-default"></span>
                          <span
                            className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold ${previewMode === "outline" ? "bg-[#6FCF97] text-white ring-2 ring-[#6FCF97]/30" : "bg-surface-active text-dim"}`}
                          >
                            2
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      {previewMode === "preview" && (
                        <>
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-7 text-[11px] bg-[#6FCF97]/10 text-[#6FCF97] border-[#6FCF97]/20 hover:bg-[#6FCF97]/20 font-semibold"
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 text-[11px] text-dim hover:text-body"
                          >
                      ...
                      {previewMode === "configure" && (
                        <div className="flex items-center gap-2">
                          <Button
                            size="sm"
                            className="h-7 text-[11px] bg-primary hover:bg-primary/90 text-white border-0 font-semibold"
                          ...
                          >
                            {isGenerating ? (
                              <>
                                <Loader2 className="w-3 h-3 mr-1.5 animate-spin" />{" "}
                                {generationStage || "Digesting Context..."}
                              </>
                            ) : (
                              <>
                          ...
                          {isGenerating && (
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-7 text-[11px] text-red-400 hover:text-red-500 hover:bg-red-400/10"
                      ...
                      {previewMode === "outline" && (
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            size="sm"
                            className="h-7 text-[11px] bg-gradient-to-r from-[#6FCF97] to-primary hover:from-[#6FCF97]/90 hover:to-primary/90 text-white border-0 font-semibold shadow-sm shadow-[#6FCF97]/20"
                          ...
                          >
                            {isGenerating ? (
                              <>
                                <Loader2 className="w-3 h-3 mr-1.5 animate-spin" />{" "}
                                {generatingPlatformIdx >= 0
                                  ? `Generating ${ALL_PLATFORMS[generatingPlatformIdx]} (${generatingPlatformIdx + 1}/${ALL_PLATFORMS.length})...`
                                  : "Preparing..."}
                              </>
                            ) : (
                              <>
                                <Sparkles className="w-3 h-3 mr-1.5" /> Produce
                                All Platforms
                              </>
                            )}
                          </Button>
                            ...
                            {activePreviewItem.url && (
                              <div className="flex items-center flex-wrap gap-x-4 gap-y-2 text-[11px] text-dim min-w-0">
                                ...
                                <span className="flex items-center">
                                  ...
                                  {
                                    activePreviewItem.content.split(/\s+/)
                                      .length
                                  }{" "}
                                  words
                                </span>
                            ...
                            <div className="flex-1 flex flex-col md:flex-row min-h-0 relative h-full">
                              <div className="flex-1 relative border-r border-default/30 h-full min-h-0">
                                <div className="absolute inset-0 overflow-y-auto p-5 md:p-7 pr-4 text-left custom-scrollbar">
                                  <FormattedText
                                    text={activePreviewItem.content}
                                  />
                                </div>
                              </div>
                              {aiSuggestions &&
                                activePreviewItem.id === "ks" && (
                                  <div className="w-full md:w-80 bg-surface-2/50 shrink-0 overflow-y-auto border-t md:border-t-0 p-5 space-y-4">
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      <h3 className="text-xs font-bold text-heading uppercase tracking-wider">
                                        {t("smart_entry.key_insights")}
                                      </h3>
                                    </div>
                                    <div className="space-y-3">
                                      {aiSuggestions.key_insights.map(
                                        (insight, idx) => (
                                          <div
                                            key={idx}
                                    ...
                                    <div className="pt-4 border-t border-default/50 space-y-3">
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-[10px] text-label font-medium truncate ml-2">
                        ...
                        <div className="max-w-xl mx-auto p-6 space-y-6">
                          ...
                          <div className="space-y-4">
                            <h3 className="text-xs font-semibold text-heading uppercase tracking-wider flex items-center gap-2">
                              ...
                              {DICTIONARY[
                                briefForm.language as keyof typeof DICTIONARY
                              ]?.contentBrief || DICTIONARY.en.contentBrief}
                            </h3>

                            {/* Generation Stage Progress Bar */}
                            {isGenerating && generationStage && (
                              <div className="rounded-xl border border-primary/20 bg-primary/[0.02] p-4 space-y-3 shadow-sm shadow-primary/5">
                                <div className="flex items-center justify-between">
                                ...
                                <div className="h-1.5 bg-surface-hover rounded-full overflow-hidden border border-white/5">
                                  <div
                                    className="h-full bg-gradient-to-r from-primary via-emerald-500 to-primary rounded-full animate-progress-shimmer"
                                    style={{
                                      width: "66%",
                            ...
                            {(imageUrlValue.trim() ||
                              ...
                              detectedImageURL) && (
                              <div className="rounded-xl border border-default/50 bg-surface-1 p-4 flex items-center gap-4 group hover:border-primary/30 transition-all duration-300 shadow-lg shadow-black/5">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2 mb-1.5">
                                    ...
                                    <span className="text-[10px] font-bold text-heading uppercase tracking-widest">
                                      {DICTIONARY[
                                        briefForm.language as keyof typeof DICTIONARY
                                      ]?.visualAnchor ||
                                        DICTIONARY.en.visualAnchor}
                                    </span>
                                  </div>
                                  <p className="text-[11px] text-dim leading-relaxed font-medium">
                                    {DICTIONARY[
                                      briefForm.language as keyof typeof DICTIONARY
                                    ]?.visualHint || DICTIONARY.en.visualHint}
                                  </p>
                                </div>
                              </div>
                            )}
                            ...
                            <div className="space-y-2">
                              <label className="text-[11px] font-bold text-dim uppercase tracking-wider ml-1">
                                {DICTIONARY[
                                  briefForm.language as keyof typeof DICTIONARY
                                ]?.targetPlatforms ||
                                  DICTIONARY.en.targetPlatforms}
                              </label>
                              <div className="rounded-xl bg-surface-1 border border-default p-4 shadow-sm">
                                <div className="flex items-center gap-2 mb-3">
                                  <div className="p-1.5 rounded-lg bg-emerald-500/10">
                                    <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
                                  </div>
                                  <span className="text-[11px] font-bold text-emerald-400 tracking-tight">
                                    Multi-Platform Content Synthesis
                                  </span>
                                </div>
                                <p className="text-[11px] text-dim leading-relaxed mb-4 font-medium opacity-80">
                                  {DICTIONARY[
                                    briefForm.language as keyof typeof DICTIONARY
                                  ]?.platformHint || DICTIONARY.en.platformHint}
                                </p>
                                  ...
                                  {[
                                    {
                                      ...
                                      icon: (
                                        ...
                                        >
                                          <path
                                            fill="#F59E0B"
                                            d="M20 2H4c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"
                                          />
                                          <path
                                            fill="#FFF"
                                            d="M16 14H8v-2h8v2zm0-4H8V8h8v2z"
                                          />
                                        </svg>
                                      ),
                                      ...
                                      icon: (
                                        ...
                                        >
                                          <g>
                                            <path
                                              fill="#1877F2"
                                              d="M15 8a7 7 0 00-7-7 7 7 0 00-1.094 13.915v-4.892H5.13V8h1.777V6.458c0-1.754 1.045-2.724 2.644-2.724.766 0 1.567.137 1.567.137v1.723h-.883c-.87 0-1.14 1.093V8h1.941l-.31 2.023H9.094v4.892A7.001 7.001 0 0015 8z"
                                            ></path>
                                            <path
                                              fill="#ffffff"
                                    ...
                                    <div
                                      key={p.label}
                                      className="flex-1 px-3 py-3 rounded-xl bg-surface-2 border border-default flex flex-col items-center gap-1.5 hover:bg-surface-active transition-colors shadow-inner"
                                    >
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span
                                          className={`text-[10px] font-bold ${p.color} uppercase tracking-tight`}
                                        >
                                          {p.label}
                                        </span>
                                      </div>
                                      <span className="text-[9px] text-dim font-semibold">
                                        {p.hint}
                                      </span>
                                    </div>
                                  ))}
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[11px] font-medium text-label flex items-center gap-2">
                                {DICTIONARY[
                                  briefForm.language as keyof typeof DICTIONARY
                                ]?.toneOfVoice || DICTIONARY.en.toneOfVoice}
                              </label>
                              <select
                                ...
                                className="w-full bg-surface-hover text-xs text-body px-3 py-2 rounded-lg border border-default outline-none focus:border-primary/40 [color-scheme:dark]"
                              >
                                {(
                                  DICTIONARY[
                                    briefForm.language as keyof typeof DICTIONARY
                                  ]?.tones || DICTIONARY.en.tones
                                ).map((t) => (
                                  <option
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div className="space-y-1.5">
                                <label className="text-[11px] font-medium text-label">
                                  {DICTIONARY[
                                    briefForm.language as keyof typeof DICTIONARY
                                  ]?.language || DICTIONARY.en.language}
                                </label>
                                <select
                                  ...
                                  className="w-full bg-surface-hover text-xs text-body px-3 py-2 rounded-lg border border-default outline-none focus:border-primary/40 [color-scheme:dark]"
                                >
                                  <option className="bg-surface-2" value="vi">
                                    🇻🇳 Tiếng Việt
                                  </option>
                                  <option className="bg-surface-2" value="en">
                                    🇺🇸 English
                                  </option>
                                </select>
                              </div>
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[11px] font-medium text-label flex items-center gap-2">
                                {DICTIONARY[
                                  ...
                                  DICTIONARY.en.targetAudience}{" "}
                                <span className="text-faint">
                                  {DICTIONARY[
                                    briefForm.language as keyof typeof DICTIONARY
                                  ]?.optional || DICTIONARY.en.optional}
                                </span>
                              </label>
                              <input
                                type="text"
                                ...
                                placeholder={
                                  ...
                                  DICTIONARY.en.audiencePlaceholder
                                }
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[11px] font-medium text-label">
                                {DICTIONARY[
                                  ...
                                  DICTIONARY.en.additionalInstructions}{" "}
                                <span className="text-faint">
                                  {DICTIONARY[
                                    briefForm.language as keyof typeof DICTIONARY
                                  ]?.optional || DICTIONARY.en.optional}
                              ...
                              <textarea
                                ...
                                placeholder={
                                  ...
                                  DICTIONARY.en.instructionsPlaceholder
                                }
                                rows={2}
                          ...
                          <div className="space-y-4 pt-4 border-t border-default/50">
                            <h3 className="text-xs font-semibold text-heading uppercase tracking-wider flex items-center gap-2">
                              ...
                              {DICTIONARY[
                                ...
                                DICTIONARY.en.advancedWriting}
                            </h3>
                            <p className="text-[10px] text-dim leading-relaxed">
                              {DICTIONARY[
                                briefForm.language as keyof typeof DICTIONARY
                              ]?.expertiseHint || DICTIONARY.en.expertiseHint}
                            </p>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                              <div className="space-y-1.5">
                                <label className="text-[11px] font-medium text-label flex items-center h-4">
                                  {DICTIONARY[
                                    briefForm.language as keyof typeof DICTIONARY
                                  ]?.pointOfView || DICTIONARY.en.pointOfView}
                                </label>
                                <select
                                  value={briefForm.point_of_view}
                                  onChange={(e) =>
                                    setBriefForm((f) => ({
                                      ...f,
                              ...
                              <div className="space-y-1.5">
                                <label className="text-[11px] font-medium text-label flex items-center h-4">
                                  {DICTIONARY[
                                    ...
                                    DICTIONARY.en.expertiseLevel}
                                </label>
                                <select
                                  value={briefForm.expertise_level}
                                  onChange={(e) =>
                                    setBriefForm((f) => ({
                                      ...f,
                                ...
                                >
                                  {(
                                    DICTIONARY[
                                      briefForm.language as keyof typeof DICTIONARY
                                    ]?.expertise || DICTIONARY.en.expertise
                                  ).map((ex) => (
                                    <option
                                      className="bg-surface-2"
                                      key={ex.value}
                              ...
                              <div className="space-y-1.5 md:col-span-1 col-span-2">
                                <label className="text-[11px] font-medium text-label flex items-center gap-2 h-4">
                                  {DICTIONARY[
                                    ...
                                    DICTIONARY.en.copywritingFramework}
                                </label>
                                <select
                                  value={briefForm.framework}
                                  onChange={(e) =>
                                    setBriefForm((f) => ({
                                      ...f,
                                ...
                                >
                                  {(
                                    DICTIONARY[
                                      briefForm.language as keyof typeof DICTIONARY
                                    ]?.frameworks || DICTIONARY.en.frameworks
                                  ).map((fr) => (
                                    <option
                                      className="bg-surface-2"
                                      key={fr.value}
                      ...
                      <div className="flex-1 overflow-y-auto">
                        <div className="max-w-2xl mx-auto p-6 space-y-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-[10px] text-dim mt-1">
                                Review and adjust. The AI will follow this
                                outline precisely.
                              </p>
                            </div>
                            <Badge
                          ...
                          <textarea
                            ...
                            className="w-full h-[500px] bg-surface-hover text-body font-mono text-[11px] p-4 rounded-lg outline-none focus:ring-1 focus:ring-primary/40 resize-y border border-default/30"
                            spellCheck={false}
                          />
                          <div className="flex items-center gap-2 text-[10px] text-faint">
                            <AlertTriangle className="w-3 h-3 text-amber-500" />
                            Edit the JSON to tweak headings, key points, or data
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-20 text-dim">
                  ...
                  <p className="text-xs text-faint mt-1">
                    Ready for knowledge extraction and AI processing
                  </p>
                </div>
              )}
            </div>
...
function StatusBadge({
  ...
  return (
    ...
    >
      <div className="flex-shrink-0 flex items-center pr-1.5">{icon}</div>
...
function FormattedText({ text }: { text?: string }) {
  if (!text)
    return <p className="text-dim text-xs italic">No content available.</p>;
      ...
      {text.split("\n\n").map((block, idx) => {
        ...
        if (content.startsWith("# "))
          return (
            <h1
              key={idx}
              className="text-xl font-semibold text-heading tracking-tight mt-6 mb-3"
            >
              {content.replace(/^#\s+/, "")}
            </h1>
          );
        if (content.startsWith("## "))
          return (
            <h2
              key={idx}
              className="text-lg font-semibold text-heading tracking-tight mt-5 mb-2"
            >
              {content.replace(/^##\s+/, "")}
            </h2>
          );
        if (content.startsWith("### "))
          return (
            <h3
              key={idx}
              className="text-base font-semibold text-heading mt-4 mb-2"
            >
              {content.replace(/^###\s+/, "")}
            </h3>
          );
          ...
          return (
            <ul key={idx} className="list-disc pl-5 space-y-1 my-3">
              {content.split("\n").map((item, i) => (
                <li
                  key={i}
                  className="text-[13px] text-body whitespace-pre-wrap"
                >
        ...
        if (imgRegex.test(content)) {
              ...
              elements.push(
                <span key={`img-${i}`} className="block my-5 text-center">
                  <img
                    ...
                    alt={alt}
                    loading="lazy"
                    className="mx-auto max-h-[400px] object-contain rounded-lg shadow-sm border border-default/20 bg-surface-1"
                  />
                  {alt && (
                    <span className="block text-[11px] text-dim mt-2 italic">
                      {alt}
                    </span>
                  )}
                </span>,
          ...
          return (
            <div key={idx} className="text-[13px] leading-relaxed text-dim">
              {elements}
            </div>
          );
        }
        ...
        return (
          <p
            key={idx}
            className="text-[13px] leading-relaxed text-dim whitespace-pre-wrap"
          >
            {formatBold(content)}
          </p>
        );
